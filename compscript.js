...
let playerName = "Player"; // Replace "Player" with the actual player's name
    ...
    for (let con of wincon){
        ...
        if(cell[a].innerText !== "" && cell[a].innerText === cell[b].innerText && cell[a].innerText === cell[c].innerText){
            if(cell[a].innerText === "X"){
                ...
                resultc.innerText = `${playerName}, you win! You defeated the computer.
                Your Winning combination: ${con}`;
            } else {
                p2s.innerText = ++player2Count;
                resultc.innerText = `${playerName}, you were defeated by the computer.
                Winning combination: ${con}`;
            }
            reset();
            return;
        }
